...
def show_data(car)
  ...
  puts "乗車人数(人)を入力して下さい。"
  input_number = gets.to_f

  calculate_capacity(car, input_number)
...
def calculate_capacity(car, input_number)
  nokori = car[:capacity] - input_number 
    if nokori > 0 
      puts "あと#{nokori}人乗れます。"
    elsif nokori == 0
      puts "満員です。"  
    else 
      puts "#{nokori.abs}人の定員オーバーです。"
    end
end
